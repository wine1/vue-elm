...
export const getAddressList = user_id =>
  fetch("/v1/users/" + user_id + "/address");

// 搜索地址
export const searchPlace = (city_id, keyword) =>
  fetch("/v1/pois/", { city_id, keyword });

// 获取经纬度详细定位
export const msiteAddress = geohash => fetch("/v2/pois/" + geohash,{geohash});
...
export const foodCategory = (latitude, longitude) =>
  fetch("/shopping/v2/restaurant/categroy", {
    latitude,
    longitude
  });
...
// 获取商家属性活动列表
export const foodSctivity = (latitude, longitude) =>
...
//获取商家详情
export const restaurantDetails = (shopid, latitude, longitude) =>
...
// 获取评价列表
export const getRatingList = (shopid, tag_name, offset) =>
  fetch("ugc/v2/restaurants" + shopid + "rating", {
    has_content: true,
    offset,
    limit: 10,
    tag_name
...
// 获取备注信息
export const getMarks = cartid => fetch("/v1/carts/" + cartid + "remarks");

// 获取收货地址列表
export const getAddress = userid => fetch("/v1/users/" + userid + "/addresses");
